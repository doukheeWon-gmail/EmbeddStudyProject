...
#project version make
PROVERSION = $(VERSION)$(if $(SUB_VERSION),.$(SUB_VERSION))

#get this project version
projectversion:
	@echo project version is v$(PROVERSION)

#create build object file folder
createdirs:
	@echo create build dirctory $(OUT_DIR)
	@echo bin file in $(BUILD_DIR)
	@echo library file object in $(LIB_DIR)
...
#Create format hex file
%.hex: %.elf
	$(OBJCOPY) -O $(FORMAT) $< $@

#Creat format binary file
%.bin: %.elf
	$(OBJCOPY) -O $(FORMAT) $< $@
...
.PHONY : all finish end sizeinfo projectversion gccversion createdirs build elf hex bin lss sym clean