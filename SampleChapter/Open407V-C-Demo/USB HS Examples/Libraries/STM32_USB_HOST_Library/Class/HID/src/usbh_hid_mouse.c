...
{  
  char temp[100];
  HID_MOUSE_Data.button = data[1];

  HID_MOUSE_Data.x      = data[2];
  HID_MOUSE_Data.y      = data[3];
  
  sprintf(temp, "button:0x%x", (char *)HID_MOUSE_Data.button);
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 130, (void *)temp );
  sprintf(temp, "x:%d", (char *)HID_MOUSE_Data.x);
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 115, (void *)temp );
  sprintf(temp, "y:%d", (char *)HID_MOUSE_Data.y);
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 100, (void *)temp );
